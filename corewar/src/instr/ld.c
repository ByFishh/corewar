...
static void clc_carry(champ_t *champ, data_t *data)
{
...
void ld_mnemonic(champ_t *champ, data_t *data)
{
    int tm = 0;
    ...
    if (store.param_types[0] == 2)
        ...
        = data->arena[(init + tm % IDX_MOD) % MEM_SIZE];
    clc_carry(champ, data);
    champ->cycle += 5;
}