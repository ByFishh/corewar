...
static int xor_error(char **parse, char **args, int **arg_pos)
{
    int i = 1;
...
static void xor_add_node(data_t *data, char **parse, char *args)
{
    int binary = 0;
    node_t *tmp = NULL;
...
int xor_instr(data_t *data, char **parse)
{
    ...
    if (!xor_error(parse, &args, &arg_pos))
        return (0);
    if (parse[1][1] == LABEL_CHAR) {
        if (!verif_dir_label(data, parse[1], "xor", arg_pos[1]))
            return (0);
    }
    if (parse[2][1] == LABEL_CHAR) {
        if (!verif_dir_label(data, parse[2], "xor", arg_pos[2]))
            return (0);
    }
    xor_add_node(data, parse, args);
    free(args);
    free(arg_pos);
    return (1);
}