...
static int ldi_error(char **parse, char **args, int **arg_pos)
{
    int i = 1;
...
static void ldi_add_node(data_t *data, char **parse, char *args)
{
    int binary = 0;
    node_t *tmp = NULL;
...
int ldi_instr(data_t *data, char **parse)
{
    ...
    if (!ldi_error(parse, &args, &arg_pos))
        return (0);
    data->have_label = my_memset_char(my_strlen(parse[1]));
    if (parse[1][1] == LABEL_CHAR) {
        if (!verif_dir_label(data, parse[1], "ldi", arg_pos[0]))
            return (0);
    }
    if (parse[2][1] == LABEL_CHAR) {
        if (!verif_dir_label(data, parse[2], "ldi", arg_pos[1]))
            return (0);
    }
    ldi_add_node(data, parse, args);
    free(arg_pos);
    return (1);
}