...
static int zjmp_error(char **parse, int i, char **args)
{
...
static void zjmp_add_node(data_t *data, node_t *tmp, char **parse, char *args)
{
...
int zjmp_instr(data_t *data, char **parse)
{
    ...
    if (!(i = zjmp_error(parse, i, &args)))
        return (0);
    data->have_label = my_memset_char(my_strlen(parse[1]));
    if (parse[1][1] == LABEL_CHAR) {
        if (!verif_dir_label(data, parse[1], "zjmp", 2))
            return (0);
    }
    zjmp_add_node(data, tmp, parse, args);
    return (1);
}