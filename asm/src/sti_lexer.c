...
static int sti_error(char **parse, char **args, int **arg_pos)
{
    int i = 1;
...
static void sti_add_node(data_t *data, char **parse, char *args)
{
    int binary = 0;
    node_t *tmp = NULL;
...
int sti_instr(data_t *data, char **parse)
{
    ...
    if (!sti_error(parse, &args, &arg_pos))
        return (0);
    data->have_label = my_memset_char(my_strlen(parse[1]));
    if (parse[2][1] == LABEL_CHAR) {
        if (!verif_dir_label(data, parse[2], "sti", arg_pos[1]))
            return (0);
    }
    if (parse[3][1] == LABEL_CHAR) {
        if (!verif_dir_label(data, parse[3], "sti", arg_pos[2]))
            return (0);
    }
    sti_add_node(data, parse, args);
    free(arg_pos);
    return (1);
}